...
若你是在中國大陸，並且户籍也是中國大陸，那麼選擇其實非常多。例如阿里雲、騰訊雲、華為雲、百度雲等。要注意的是這些雲計算平台通常有非常猛的一次性新用户優惠。最瘋狂的情況下可以以 1 折的價錢購買到手，如果是打算長時間和朋友一起遊玩，可以考慮一次性購買 2 年，甚至 3 年。

如果想要一直用這些優惠，在備份好的情況下，可以每幾個月，甚至幾年換一次平台。

大陸以外的國家則有更多考慮，VPS 平台有很多，不再廢話。不過我就只能很慘地用阿里雲，所以本文有些地方可能會主要以阿里雲為主來説明，但概念上其實是一樣的，其它平台也應該會有一樣的東西，自己稍微變通一下再進行設置即可。

## 為什麼要 VPS？

你也可以只在自己的電腦上 (Windows 10) 運行伺服端，但中國大陸的網絡多數不會有公網 IP，so...

VPS 除了有提供公網 IP 之外，也可以 7x24 運行，擁有一台自己的 VPS 也可以用來做很多事，比如 steam 掛卡，或是架設其它遊戲的伺服端。

> 如果你確實只打算在自己的電腦上運行，可以跳過 VPS 部分。畢竟測試伺服端時我也是在自己電腦上測試。
...
> 此處的「運行」是單純地指運行，網絡的部分我會在優化篇再寫，如果單純只和大陸以內的朋友聯機，通常情況下不需要擔心。
...
如果 Mod 比較多，就要使用 2C4G 的配置。如果 Mod 中有比較消耗性能的，也可以考慮 4C4G。此外，聯機人數越多，所需的 RAM 越高。RAM 的需求可以簡單地視作「每個人約 100 M」。
...
為了方便地進行管理，你需要以 SSH 的方式連接到 VPS。而登入 VPS 默認是用密碼的，你**最好**改成密鑰方式登入。
...
```
java -d64 -server -XX:+UseG1GC -XX:MaxGCPauseMillis=100 -XX:+UseStringDeduplication -Xms1G -Xmx2560M -XX:hashCode=5 -Dfile.encoding=UTF-8 -jar Minecraft_Server.jar nogui
pause
```

> 這是我平時在使用的指令。JVM 初始分配 1G RAM，最多分配到 2560M，也就是 2.5G。這兩個參數都可以修改，但最好不要小於 1G。
>
> 參數中的 Minecraft_Server.jar 並不是一成不變的，根據你使用的伺服端 .jar 文件需要更改。
>
> 後面的 nogui 就是在 Windows 下不使用 GUI，因為不需要。
>
> 最後的 pause 是默認以 Powershell 或 Cmd 執行完指令後，不讓窗口直接消失而添加的，在 VPS 上使用 linux 操作系統時，可以去掉。
...
> 通常情況下，配置文件我會手動添加一行 `spawn-protection=3`，不知道為什麼這行默認不會生成。如果出生點被 Creeper 破壞的話會非常煩。
...
我最早是在 beta 1.7.3 開始接觸到這種伺服端的，那個時候的伺服端實際上要更複雜，是一種同時支援插件與模組的伺服端，這個接下來也會講。當時能安裝的模組只有 Ported 過的，只有一些特定的大型模組。但在那個時候能夠玩到最早版本的 IndustrialCraft 已經非常棒了。如果我沒有記錯的話，那是叫 MCPC+。直到 1.7.10 （注意是正式版）之前，伺服端幾乎都是同時支援插件與模組的方式。非常有名的是 Cauldron->KCauldron->Thermos 這系列。

### Forge，非常普遍的 Modded Server
...
嗯……我就直接稱為混合端了。要注意的是，融合端全部都是在基於 Bukkit 系的情況下支援 Mod。
...
Liteloader 正如其名，它很 Lite，在 1.11 開始出現，終止於 1.12。當時能夠兼容 Forge，而 Liteloader 上的 Mod 基本上都是非常輕量的 Mod，像是自動釣魚之類的。
...
因此，這種情況下報錯的 Mod 反而會出現在第一行，這就非常好定位了，刪了它。
...
然後，伺服端啟動的情況下，控制台內使用指令新建一個默認玩家組，向這個組的玩家添加你想要玩家默認能用的權限，包括但不僅限於 `/tpa` 、 `/home` 之類的，以及部分插件提供的權限。

之後，新建一個 admin 組，繼承自上面的玩家組，將所有權限添加到 admin 組。這樣就簡單地區分開了普通玩家和 Operators。
...
如果伺服器所在位置和玩家所在位置接近，或者是在一個國家內，問題通常不大。但如果跨國，或是跨太平洋等，問題就大了。網絡路由並不是呈直線的。
...
此處的硬性要求是，B 到 A 的路由質量要高。這樣我們就可以在 VPS B 上插個木牌，告訴玩家可以從這裏連接到 A。
